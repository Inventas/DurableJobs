...
DurableJobs and Point-Free SQLiteData can share one GRDB `DatabasePool`.
DurableJobs uses that writer for its own `durable_queue_*` tables. SQLiteData
...
func makeQueue(
    ...
    // The initializer runs the DurableJobs initial schema migration on this
...
The fixture assumes the host app, not DurableJobs, owns the SQLiteData
...
minimum OS and Swift toolchain. Do not add SQLiteData to DurableJobs's
...
its iOS 15-compatible StructuredQueries line without changing DurableJobs's
dependency graph. Build it from `Examples/SQLiteDataIntegration`.

## Schema and reliability

DurableJobs's initial queue migration creates tables with a
